...
export class ArduinoUno {
  ...
  public simulationTimeCallback: callback | null = null;
  ...
  executeProgram(hex: string) {
    ...
    for (const event of this.cpuEvents) this.runner.addCPUEvent(event);

    for (const connection of this.pinConnections) connection.component.reset();

    this.runner.portD.addListener((value) => {
      if (this.runner) this.updateComponents(value, 0, this.runner.cpu.cycles);
    });
    this.runner.portB.addListener((value) => {
      if (this.runner) this.updateComponents(value, 8, this.runner.cpu.cycles);
    });
    ...
    this.runner.execute((cpu) => {
      ...
      if(this.simulationTimeCallback)
        this.simulationTimeCallback(time);
    });
  }
