...
export function findMinimumDistanceToObstacle(startingPosition: any,startingAngle : number, maximumDistance : number, obstacles:Array<Body>)
{
    let minDistance = maximumDistance + 10000;

    const numberOfRays = 15;

    for(var i =0; i < numberOfRays; i+=1)
    {
      const endPoint = getTranformedPoint(startingPosition, startingAngle-i*Math.PI/180, maximumDistance,0);
